...
for likelihood in likelihood_list:
    ...
    if type(settings.likelihood).__name__ == 'exp_power':
        root += "_" + str(settings.likelihood.power).replace('.', '_')
    ...
    # override limits in certain cases
    if settings.n_dim == 10:
        if type(settings.likelihood).__name__ == "gaussian":
            xmin = -35
            ymax = 1200
        elif type(settings.likelihood).__name__ == "exp_power":
            if settings.likelihood.power == 2:
                xmin = -40
                ymax = 2000
            elif settings.likelihood.power == 0.75:
                xmin = -30
                ymax = 1000
        elif type(settings.likelihood).__name__ == "cauchy":
            xmin = -45
            ymax = max(standard_nlive_max * 4, w_an.max() * 1.05)
    ax.set_xlim([xmin, 0])
    ax.set_ylim([0, ymax])
